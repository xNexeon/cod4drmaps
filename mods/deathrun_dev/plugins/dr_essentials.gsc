...
buildMenuOptions( role )
{
	...
	if( level.dvar["essentials_menu_points_enabled"] == 1 )
	{
		pm_addSub( "Buy Weapon", "give_weap" );
			pm_addOpt( "M9 ("+level.dvar["price_m9"]+" pts)",          "give_weap", ::ess_weap_m9 );
			pm_addOpt( "M1911 ("+level.dvar["price_m1911"]+" pts)",    "give_weap", ::ess_weap_m1911 );
			pm_addOpt( "USP ("+level.dvar["price_usp"]+" pts)",        "give_weap", ::ess_weap_usp );
			pm_addOpt( "Deagle ("+level.dvar["price_deagle"]+" pts)",  "give_weap", ::ess_weap_deagle );
			pm_addOpt( "Gold Deagle ("+level.dvar["price_gold_deagle"]+" pts)", "give_weap", ::ess_weap_gold_deagle );
			pm_addOpt( "Colt 44 ("+level.dvar["price_colt44"]+" pts)", "give_weap", ::ess_weap_colt44 );
			pm_addOpt( "M40A3 ("+level.dvar["price_m40a3"]+" pts)",    "give_weap", ::ess_weap_m40a3 );
			pm_addOpt( "R700 ("+level.dvar["price_r700"]+" pts)",      "give_weap", ::ess_weap_r700 );
	}

	// ---- VIP AND ABOVE ----
	if( roleAtLeast(role,"vip") )
	{
		pm_addSub( "^5Speed", "sped" );
		...
		pm_addSub( "^5Models", "mod" );
			pm_addOpt( "Cpt. Price",  "mod", ::model_usmccqb );
	...
	{
		pm_addSub( "^3Administration", "adm" );

		// Round / Map control (direct actions in adm submenu)
		pm_addOpt( "^2End Round",        "adm", ::adm_endround );
		pm_addOpt( "^2Finish Map",       "adm", ::adm_endmap );
		pm_addOpt( "Spawn All",          "adm", ::adm_spawnall );
		...
		pm_addOpt( "Nuke Bullets",       "adm_fun", ::ess_nuke );
		...
		{
			pm_addOpt( "Ghost Mode",      "adm_fun", ::ess_ghost );
			pm_addOpt( "Matrix Effect",   "adm_fun", ::ess_matrix );
			pm_addOpt( "Party Mode",      "adm_fun", ::ess_party );
			pm_addOpt( "Throwing Knives", "adm_fun", ::ess_throw );
			pm_addOpt( "Clones",          "adm_fun", ::ess_clones );
		}

		if( role == "owner" )
		{
			pm_addOpt( "Give All 500 XP",    "adm_fun", ::own_xp_all500 );
			pm_addOpt( "Head Explode (Self)", "adm_fun", ::ess_RemoveYoHead );
		}

		// Manage Players submenu linked from adm
		pm_addSubTo( "^3Manage Players", "adm", "adm_players" );
		self adm_buildPlayerList();

		// Debug tools — owner only
		if( role == "owner" )
		{
			pm_addSub( "^1Debug", "dbg" );
			pm_addOpt( "Get Trig Coords",  "dbg", ::adm_getEndTrigCoords );
			pm_addOpt( "Test Trig Here",   "dbg", ::adm_testEndTrigHere );
			pm_addOpt( "Remove Test Trig", "dbg", ::adm_removeTestTrig );
		}
	}
}
...
// Adds a submenu link to a SPECIFIC parent menu (for nested tabs inside adm etc.)
pm_addSubTo( displayname, parent, name )
{
	pm_addOpt( displayname, parent, name );
}
...
adm_xp_self()
{
	amount = 50; // Change this number to however many times you want it to run
	...
	iPrintln("^3[Admin] ^7" + self.name + " ^7got ^3" + (1500 * amount / 2) + " XP!");
}
adm_xp_all()
{
	players = getEntArray("player","classname");
...
//=============================================================================
// ADMIN: END ROUND / FINISH MAP
//=============================================================================
adm_endround()
{
	iPrintln("^3[Admin] ^7"+self.name+" ^7ended the ^1round!");
	braxi\_mod::endRound("Administrator ended round", "jumpers");
}

adm_endmap()
{
	iPrintln("^3[Admin] ^7"+self.name+" ^7is finishing the ^1map!");
	braxi\_mod::endMap("Administrator ended game");
}

//=============================================================================
// ADMIN: PLAYER MANAGEMENT
// Builds a per-player submenu listing everyone online.
// Selecting a player opens an action submenu for that player.
//=============================================================================
adm_buildPlayerList()
{
	self.adm_targets = [];
	...
	for(i=0;i<players.size;i++)
	{
		idx = self.adm_targets.size;
		self.adm_targets[idx] = players[i];
